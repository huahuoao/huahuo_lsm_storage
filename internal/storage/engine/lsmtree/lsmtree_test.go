...
func Example() {
	...
	tree, err := Open(dbDir, SparseKeyDistance(64), MemTableThreshold(1000000))
	if err != nil {
...
func TestPutForErrors(t *testing.T) {
	...
	if !errors.Is(err, ErrKeyRequired) {
		t.Fatalf("expected %v, but got %v", ErrKeyRequired, err)
	}
	...
	if !errors.Is(err, ErrValueRequired) {
		t.Fatalf("expected %v, but got %v", ErrValueRequired, err)
	}
	...
	if !errors.Is(err, ErrKeyTooLarge) {
		t.Fatalf("expected %v, but got %v", ErrKeyTooLarge, err)
	}

	var largeValue [65536]byte
	err = tree.Put([]byte("some key"), largeValue[:])
	if !errors.Is(err, ErrValueTooLarge) {
		t.Fatalf("expected %v, but got %v", ErrValueTooLarge, err)
	}
}
